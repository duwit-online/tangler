...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (!isLogin) toast.success('Account created! Complete your profile to start matching.');
        navigate('/');
      }
    } catch {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-10 right-[-30px] w-40 h-40 rounded-full bg-primary/8 blur-3xl" />
      <div className="absolute bottom-20 left-[-20px] w-32 h-32 rounded-full bg-accent/8 blur-3xl" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 relative z-10">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground font-medium">Connect Now</p>
        </motion.div>
            ...
            <div className="flex bg-secondary/60 rounded-2xl p-1 mb-6">
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                  isLogin ? 'bg-card text-foreground shadow-card' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                Sign In
              </button>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`pl-10 h-12 rounded-xl bg-secondary/50 border-border/50 text-sm focus:bg-card transition-colors ${errors.email ? 'ring-2 ring-destructive' : ''}`}
                  />
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`pl-10 pr-10 h-12 rounded-xl bg-secondary/50 border-border/50 text-sm focus:bg-card transition-colors ${errors.password ? 'ring-2 ring-destructive' : ''}`}
                  />
                  <button
                    type="button"
